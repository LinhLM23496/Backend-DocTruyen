...
import { deleteUserById, getUserById, getUsers } from '~/models/database/User'
...
export const deleteUser = async (req: Request, res: Response) => {
  try {
    ...
    return res.status(HttpStatus.OK).json({ error: 0, data: deleteUser, message: Messages.DELETE_USER_SUCCESS })
  } catch (error) {
    return res
      .status(HttpStatus.INTERNAL_SERVER_ERROR)
      .json({ error: 1, message: Messages.HTTP_500_INTERNAL_SERVER_ERROR })
